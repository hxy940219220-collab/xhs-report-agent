...
从报告导入到人工确认发布，核心工作流集中在四个连续步骤；AI 模型接入则作为可选能力独立配置。

### 1. 上传首页

从上传 PDF 或 Word 文档开始。文档仅在本机解析；首页也会保留最近任务，方便继续未完成的内容。

![上传首页界面](docs/screenshots/upload-homepage.png)

### 2. 文案与封面审核

在同一工作台选择标题、修改正文与标签，并实时预览可编辑的报告封面。

![文案与封面审核界面](docs/screenshots/copy-cover-workbench.png)

### 3. 报告整页图片选择

PDF 会按页生成完整图片，可按发布顺序选择最多 12 张报告页，也支持按需手动裁切。

![报告整页图片选择界面](docs/screenshots/report-image-selection.png)

### 4. 最终审核与同步发布

审核封面、标题、正文和图片顺序后，选择人数较少的群聊与定时发布时间，再同步至官方编辑页做最终发布确认。

![最终审核与同步发布界面](docs/screenshots/review-and-publish.png)

### 可选：AI 接入设置

提供丰富模型API接入，兼容硅基流动、Deepseek、GLM、Qwen、Kimi、 OpenAI等模型供应商，API Key 仅加密保存在本机。
